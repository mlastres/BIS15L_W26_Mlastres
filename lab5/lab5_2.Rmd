...
1. Load the data
```{r}
fish_data<- read.csv("~/Desktop/BIS15L_W26_Mlastres/lab5/data/Gaeta_etal_CLC_data.csv")
```
```{r}
names(fish_data)
```
...
shift command M
select and filter (select goes first)
```{r}
penguins %>% 
...
penguins %>% 
  select(species, body_mass_g) %>% 
  filter(species=="Gentoo") %>% 
  slice_min(body_mass_g, n=1) #dont forget to specify the variable
```
