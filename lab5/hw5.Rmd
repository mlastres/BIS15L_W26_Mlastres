...
```{r}
homerange<- read.csv("/Users/Miala/OneDrive/Desktop/BIS15L_W26_Mlastres/lab5/data/Tamburelloetal_HomeRangeDatabase.csv")
```
...
ggplot(data=homerange,
  ...
  labs(title = "number of animals in each class")
```
...
ggplot(data=homerange,
  ...
  geom_bar(mapping = aes(fill=class))+
  labs(title = "number of animals in each guild")
```
...
```{r}
homerange_birds_mammal<- homerange %>% 
  select(class, mean.hra.m2) %>% 
filter(class == "mammalia" |class== "aves")
  
```
```{r}
 ggplot(data=homerange_birds_mammal,
...
comparison between the total mass of bodyweight between mammalia and reptilia
```{r}
 homerange_mandr<- homerange %>% 
  select(class, mean.mass.g) %>% 
filter(class == "mammalia" |class== "reptilia")
```
```{r}
ggplot(data=homerange_mandr,
       mapping= aes(x=class, y=mean.mass.g))+
  geom_boxplot(mapping = aes(color=class))+
   scale_y_log10()+
labs(title = "")
```
