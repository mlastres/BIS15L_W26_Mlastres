...
heartrate_long %>% 
  ...
  ggplot(mapping=aes(x=drug, y= rate))+
...
```{r}
relig_income <- read_csv ("~/Desktop/BIS15L_W26_Mlastres/lab9/data/relig_income.csv")
relig_income
```
...
billboard %>% 
  pivot_longer(
    col=starts_with("wk"),
    names_to = "week",
    names_prefix = "wk", #gets rid of wk in wk1, wk2, etc
    values_to = "rank",
    values_drop_na = T
  )
```
...
```{r}
plant_data<- read_csv("~/Desktop/BIS15L_W26_Mlastres/lab9/data/plant_data.csv")
plant_data
```
...
qpcr_tidy %>% 
  ...
  select(gene, experiment=e_number, replicate=r_number, 'mRNAexpression')
```
