...
```{r message=FALSE, warning=FALSE}
bears<- read_csv("~/Desktop/BIS15L_W26_Mlastres/lab14/data/bear-sightings.csv") %>% 
  clean_names()
```
...
```{r}
mapb<- get_stadiamap(bbox2, maptype = "stamen_terrain", zoom=5)
```
...
```{r}
ggmap(mapb) +
  geom_point(data = bears, aes(longitude, latitude), size=0.75, color="black", alpha=0.8)+
  labs(x= "longitude", y="latitude", title= "alaska bear records")
```
```
