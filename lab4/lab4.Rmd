...
ggplot(data=penguins, #this specifies the data
       mapping = aes(x=species, y=body_mass_g))#mao the aesthetcs
```
...
ggplot(data=penguins,
       mapping= aes(x=body_mass_g, y=flipper_length_mm,))+
         geom_point(mapping=aes(color=species, shape=island))+
  geom_smooth(method = lm, se=T)+
  labs(title="body mass (g) vs. flipper length (mm)", x='body mass (g)', y="flipper length")
```
...
names(penguins)
```
how many individuals per species? (x= species, y= individuals)
```{r}
ggplot(data = penguins,
