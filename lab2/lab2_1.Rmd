...
```{r}
x <-42 #this means x gets value 42 arrow is option -
```
...
```{r}
z=10 #this will work, but will create problems eventually, alwas use option-
```
...
```{r}
my_integer <- as.integer(my_numeric) #important when working with plots
```
...
```{r}
new_vector <- c(7, 6.2, 5, 9, NA, 4, 9.8, 7, 3, 2) #creates a new vector, series of numbers
```
...
one NA value, there will be no calculations form R
