...
the paper was meant to discover the effects of harvesting wild populations has on the evolution of mega fuana. the data is going to be about elphants, their bdoyweight, tusk size, body size, age, as well as the years the elephants were born.

**2. Load `elephants.csv` and store it as a new object called `elephants`.** 
```{r}
elephants<-read.csv("C:/Users/Miala/OneDrive/Desktop/BIS15L_W26_Mlastres/lab7/data/elephants.csv")
```
...
filter(sex== "f", years_of_sample_collection== "1966-68") %>% 
  summarize(mean_estimated_age_yr=mean(estimated_age_years, na.rm=T),
             median_estimated_age_yr=median(estimated_age_years, na.rm=T),
            sd_estimated_age_yr=sd(estimated_age_years, na.rm=T)) 
...
elephants %>% 
  ...
  labs(title= "AGE VS. TUSK SIZE (CM)", x= "age", y= "tusk size (cm)")
```
...
elephants %>% 
  ...
  summarize(avg_tusk_lenth_cm_pre= mean(tusk_length_in_cm, na.rm=T))
```
```{r}
elephants %>% 
  select(tusk_length_in_cm, years_of_sample_collection) %>% 
  filter(years_of_sample_collection=="2005-13") %>% 
  summarize(avg_tusk_lenth_cm_post= mean(tusk_length_in_cm, na.rm=T))
```

this is true, in the years 2005-13, the tusk length is lower 
...
```{r}
 elephants_mature<- elephants %>% 
  select(sex, estimated_age_years, tusk_length_in_cm) %>% 
...
elephants_mature %>% 
  ggplot(mapping = aes(x= sex, y= tusk_length_in_cm))+
           geom_boxplot()+
  labs(title=" male and female tusk lengths")
