...
sharks %>% 
  ...
  labs(title = "Number of incidents in a month",
       x= "Month",
       y= "Incidents" )
```
...
sharks %>% 
  ...
  count(time, sort=T,) %>% 
    arrange(desc(n)) %>% 
  slice(c(1,2,3,4,5,6)) %>% 
  ggplot(aes(x=time, y= n),na.rm=T)+
     geom_col()+
  labs(title = "Top 5 times in which incidents happen (and NA)", 
       x="Time", 
       y=" Incidents")
#how do i remove the na from this </3
...
sharks %>% 
  ...
  labs(title = "Incidents From Each Activity", 
       y= "Incidents",
       x= "Activities")+
...
sharks %>% 
  ...
  labs(title = "Incidents from great white sharks and their severity", x= "Injury", y= "Incidents")
```
