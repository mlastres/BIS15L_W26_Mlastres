...
```{r}
elephants %>% 
  summarize(where(age_estimated_avg=mean(estimated_age_years),
                  shoulder_hoghst_avg=mean(shoulder_height_in_cm),
                  tusk_length_avg=mean(tusk_length_in_cm),
                  tusk_circumference_in_cm_avg=mean(tusk_circumference_in_cm))
...
msleep %>% 
  group_by(vore) %>% 
  summarize(avg_bodywt=mean(bodywt, na.rm=T),
            max_bodywt=min(bodywt, na.rm=T),
...
msleep %>% 
  group_by(order) %>% 
  summarize(avg_brainwt=mean(brainwt))
```
...
msleep %>% 
  group_by(order) %>% 
  summarize(avg_brainwt=mean(brainwt, na.rm=T))
```
is ther not enough data for cetacea?
