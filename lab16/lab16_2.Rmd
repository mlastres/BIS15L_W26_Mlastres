...
```{r}
genome01_blastin <- read_csv("data/blast_results/genome01_blastin.csv") %>% 
  clean_names()
```

```{r}
summary(genome01_blastin)
```
...
```{r}
Ecoli<-readAAStringSet("data/e.coli_cds.fasta")
```
...
```{r}
blastp_results<-read.csv("data/blast_results/blastp_results.csv")
```
