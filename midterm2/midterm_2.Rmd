...
server <- function(input, output, session) {
   output$plot <- renderPlot({
      ...
      ggplot(aes(x= popn,
                 y= un_creat_ratio,
                 fill= .data[[input$fill]]))+#had to give the graph an x value so i added that here
      geom_boxplot(alpha=0.75)+#I needed to put the fill as the variable, so i put that down here as well as make the plot the correct plot (boxplot not bar/col)
       labs(title = "Uncreat ratio in each population", x="Population", y="variable")
   })
}
