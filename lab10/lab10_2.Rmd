...
```{r}
p+geom_point(size=3, color="black", alpha=0.6, shape=24, fill="lemonchiffon")
```
...
homerange %>% 
  ...
  coord_flip()+
  geom_bar()+
  labs(title = "Observation by Taxon", x= "taxonomic Group", y= NULL) #take away the y axis label
```
...
homerange %>% 
  ggplot(aes(x=taxon))+
  coord_flip()+
  geom_bar(aes(fill=trophic_guild))+
  labs(title = "Observation by Taxon", x= "taxonomic Group", y= NULL)
```
...
homerange %>% 
  ggplot(aes(x=taxon))+
  coord_flip()+
  geom_bar(aes(fill=trophic_guild), position = "dodge")+
  labs(title = "Observation by Taxon", x= "taxonomic Group", y= NULL)
```
...
```{r}
homerange %>% 
  ggplot(aes(x=class, y=log10_mass)group_by())+
  geom_boxplot()
```
