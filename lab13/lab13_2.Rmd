...
table_A <- read_csv("~/Desktop/BIS15L_W26_Mlastres/lab13/data/table_A.csv")
table_B<- read_csv("~/Desktop/BIS15L_W26_Mlastres/lab13/data/table_B.csv")
```
...
```{r}
anti_join(table_A, table_B, by= "customer_ID")#troubleshoot problematic joins
```
...
```{r message=FALSE, warning=FALSE}
spiders<- read_csv("~/Desktop/BIS15L_W26_Mlastres/lab13/data/spider_records.csv") %>% 
  clean_names()
spider_location<- read_csv("~/Desktop/BIS15L_W26_Mlastres/lab13/data/spiders_locations.csv") %>% 
  clean_names()
names(spider_location)
names(spiders)
```
...
```{r}
spiders<- full_join( spiders,spider_location, by= "accession")
```
...
```{r}
write.csv(spiders, file= "spiders_with_locations.csv")
```
